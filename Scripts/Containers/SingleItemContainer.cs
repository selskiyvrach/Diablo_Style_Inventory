...
namespace D2Inventory
{
    public class SingleItemContainer : ContainerBase
    {
        ...
        public override Projection GetProjection(InventoryItem item, Vector2 screenPos)
        {
            ...
            if(lastProjection.FieldsEqual(screenRect.NormalizedRectToScreenRect(new Rect(0,0,1,1)), canPlace, screenRect.Rect.center, content, null))
                return Projection.SameProjection;
            else 
                return lastProjection = new Projection(this, screenRect.NormalizedRectToScreenRect(new Rect(0,0,1,1)), canPlace, screenRect.Rect.center, content, null);
        }
